...
注册成功（REGISTER 200 OK）后，系统 SHALL 将续约调度作为独立的续约定时器启动（存入 `renewalTasks` Map），并在 `expires × 2/3` 时发送 REGISTER refresh。续约失败（超时或 4xx）时，系统 SHALL 取消心跳定时器，将 `downLinkStatus` 置为 `OFFLINE`，并调用 `handleFailure(configId)` 进入指数退避重新注册。

#### Scenario: 注册成功同时启动续约定时器
- **WHEN** `SipRegistrationClient` 收到 REGISTER 200 OK
- **THEN** 同时启动心跳定时器（60s 周期）和续约定时器（expires×2/3 延迟），两者独立运行
